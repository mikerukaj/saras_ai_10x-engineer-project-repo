...
- [ ] T072 [US6] Regenerate frontend API types (`frontend/src/api/schema.ts`) and extend the typed client in `frontend/src/api/client.ts` to cover the updated `Prompt` shape and new tag endpoints (depends on T070, T010) — **blocked, not attempted**: same gap as T057 — T010 (typed API client) and everything upstream of it in Phase 1–2 (frontend scaffold) don't exist yet, so T072–T078 remain undone. The backend (T064, T066–T071) is complete and independently usable via HTTP in the meantime.
- [ ] T073 [P] [US6] Build a tag input component (type-to-create-or-select) in `frontend/src/components/TagInput.tsx`, used in the create and edit prompt forms — **blocked, not attempted** (see T072).
- [ ] T074 [US6] Show a prompt's tags on `frontend/src/pages/PromptListPage.tsx` and `frontend/src/pages/PromptDetailPage.tsx` — **blocked, not attempted** (see T072).
- [ ] T075 [US6] Add a tag filter control to the prompt list page wired to `GET /prompts?tags=` in `frontend/src/pages/PromptListPage.tsx` (FR-032) — **blocked, not attempted** (see T072; the backend query parameter itself is done, see T069).
- [ ] T076 [US6] Build a tags management page — list tags with `prompt_count`, rename, delete (with confirmation) — in `frontend/src/pages/TagsPage.tsx` (FR-033, FR-034) — **blocked, not attempted** (see T072).
- [ ] T077 [P] [US6] Add Vitest tests for the tag input component and tags management page in `frontend/tests/components/TagInput.test.tsx` and `frontend/tests/pages/TagsPage.test.tsx` — **blocked, not attempted** (see T072).
- [ ] T078 [US6] Add a Playwright e2e spec covering tag-on-create, filter-by-tag, rename, and delete in `frontend/e2e/prompt-tagging.spec.ts` — **blocked, not attempted** (see T072).
